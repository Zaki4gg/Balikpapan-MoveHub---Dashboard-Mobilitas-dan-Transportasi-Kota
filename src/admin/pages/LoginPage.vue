<template>
  <div class="login-page min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 px-4">
    <div class="w-full max-w-md">
      ...
      <div class="rounded-2xl border border-slate-700/50 bg-slate-800/50 backdrop-blur-xl
        ...
        <div class="bg-gradient-to-r from-cyan-500/10 to-blue-500/10 px-6 py-4 border-b border-slate-700/30">
          ...
          <p class="mt-1 text-sm text-slate-400">Masukkan kredensial Anda</p>
        </div>
        ...
        <form @submit.prevent="handleLogin" class="p-6 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              v-model="form.email"
              type="email"
              placeholder="admin@balikpapan.id"
              required
          ...
          <!-- Demo Info -->
          <div class="rounded-lg bg-slate-700/30 border border-slate-600/30 p-4 text-sm">
            <p class="font-semibold text-slate-300 mb-2">Demo Credentials:</p>
            <p class="text-slate-400">Email: <span class="text-cyan-300 font-mono">admin@balikpapan.id</span></p>
            <p class="text-slate-400">Password: <span class="text-cyan-300 font-mono">admin123</span></p>
          </div>
        </form>
...
const handleLogin = async () => {
  ...
  const result = authService.login(form.value.email, form.value.password)
