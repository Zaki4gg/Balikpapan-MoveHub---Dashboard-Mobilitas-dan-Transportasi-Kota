  ...
  <section class="space-y-6">
    ...
    <section class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
      ...
      <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
        <article v-for="route in routes" :key="route.id" class="rounded-xl border border-slate-200 bg-slate-50 p-5">
...
import { transitRoutes } from '../../shared/data/mobilityData'
...
onMounted(async () => {
  try {
    routes.value = await api.getPublicRoutes()
  } catch {
